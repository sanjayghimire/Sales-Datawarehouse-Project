...
missing_Product_Data = df_factFinal.join(dim_product, df_factFinal.DIM_ProductId == dim_product.DIM_ProductId, "left_anti")
missing_Product_Data1 = missing_Product_Data.select("DIM_ProductId").distinct()
display(missing_Product_Data1)


missing_Employee_Data = df_factFinal.join(dim_employee, df_factFinal.DIM_EmployeeId == dim_employee.DIM_EmployeeId, "left_anti")
missing_Employee_Data1 = missing_Employee_Data.select("DIM_EmployeeId").distinct()
display(missing_Employee_Data1)

missing_Store_Data = df_factFinal.join(dim_store, df_factFinal.DIM_StoreId == dim_store.DIM_StoreId, "left_anti")
missing_Store_Data1 = missing_Store_Data.select("DIM_StoreId").distinct()
display(missing_Store_Data1)

# COMMAND ----------

df_factColCheck = df_factSales.select(
    when(col("UnitsSold").isNull(), 0).otherwise(col("UnitsSold").cast("int")).alias("UnitsSold_check"),
    when(col("UnitPrice") < 0, 0).otherwise(col("UnitPrice").cast("double")).alias("UnitPrice_check"),
    when(col("Discount").isNull(), 0).otherwise(col("Discount").cast("double")).alias("Discount_check"),
    col("SaleDate")
)

df_finalColCheck = df_factFinal.select(
    "UnitsSold", "UnitPrice", "Discount", "SaleDate"
)

df_missing = df_factColCheck.exceptAll(df_finalColCheck)
